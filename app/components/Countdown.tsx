...
export default function Countdown() {
  ...
  const units: { label: string; value: string }[] = [
    { label: "Hari", value: time ? String(time.days) : "—" },
    { label: "Jam", value: time ? pad(time.hours) : "—" },
    { label: "Menit", value: time ? pad(time.minutes) : "—" },
    { label: "Detik", value: time ? pad(time.seconds) : "—" },
  ];
  ...
  return (
    <section className="relative flex flex-col items-center bg-[color:var(--color-bark)] px-6 py-24 text-center sm:py-32">
      ...
      <Reveal delay={60}>
        <p className="eyebrow text-[color:var(--color-gold-soft)]">Menuju Hari Bahagia</p>
      </Reveal>
      <Reveal delay={100}>
