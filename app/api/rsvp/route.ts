...
export async function POST(req: NextRequest): Promise<NextResponse<RSVPResult>> {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, code: "INVALID_INPUT", message: "Isi permintaan tidak dapat dibaca." },
      { status: 400 }
    );
  }
  ...
  if (typeof body !== "object" || body === null) {
    return NextResponse.json(
      { success: false, code: "INVALID_INPUT", message: "Permintaan tidak valid." },
      { status: 400 }
    );
  }
  ...
  if (typeof guestId !== "string" || !guestId.trim()) {
    return NextResponse.json(
      { success: false, code: "INVALID_INPUT", message: "Tamu tidak dapat diidentifikasi." },
      { status: 400 }
    );
  }
  ...
  if (typeof attendance !== "string" || !ATTENDANCE_VALUES.includes(attendance as Attendance)) {
    return NextResponse.json(
      { success: false, code: "INVALID_INPUT", message: "Silakan pilih apakah Anda akan hadir." },
      { status: 400 }
    );
  }
  ...
  if (!guest) {
    return NextResponse.json(
      { success: false, code: "INVALID_GUEST", message: "Undangan Anda tidak ditemukan." },
      { status: 404 }
    );
  }
  ...
  try {
    ...
    if (err instanceof RSVPAlreadyExistsError) {
      return NextResponse.json(
        { success: false, code: "ALREADY_SUBMITTED", message: "Konfirmasi kehadiran Anda telah diterima sebelumnya." },
        { status: 409 }
      );
    }
    console.error("Failed to create RSVP", err);
    return NextResponse.json(
      { success: false, code: "SERVER_ERROR", message: "Terjadi kesalahan. Silakan coba lagi." },
      { status: 500 }
    );
  }
}
